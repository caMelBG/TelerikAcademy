...
namespace FastAndFurious.ConsoleApplication.Models.MotorVehicles.Abstract
{
    public abstract class MotorVehicle : VehiclePart, IMotorVehicle, IIdentifiable, IWeightable, IValuable
    {
        ...
        {
            int timeToAchiveTopSpeed = this.TopSpeed / this.Acceleration;

            int metersPerSecond = MetricUnitsConverter.GetMetersPerSecondFrom(this.TopSpeed);

            var timeSpan = new TimeSpan(timeToAchiveTopSpeed / metersPerSecond);
