...
namespace FastAndFurious.ConsoleApplication.Engine
{
    public class Engine
    {
        ...
        public void ExecuteCreationStrategy(string[] commandParameters)
        {
            ...
            Console.WriteLine(String.Format(GlobalConstants.CreatSuccessfullyMessage, typeName));
        }
                ...
                default:
                    {
                        throw new NotSupportedException(
                            GlobalConstants.AssigningOperationNotSupportedExceptionMessage);
                    }
            }
        ...
        {
            var trackRaceId = int.Parse(commandParameters[2]);
            var trackRace = this.raceTracks.GetById(trackRaceId);
            trackRace.RunRace();
        }

        public void ExecuteDisplayingStrategy(string[] commandParameters)
        {
            var trackRaceId = int.Parse(commandParameters[6]);
            var trackRace = this.raceTracks.GetById(trackRaceId);
            var timesCount = int.Parse(commandParameters[2]);

            Console.WriteLine(
                String.Format(
                    GlobalConstants.DisplayBestNTimesEverMessage,
                    timesCount,
                    trackRace.TrackName));
            var array = trackRace.FinishedRacesResults.ToArray();

            foreach (var participant in array[0])
            {
