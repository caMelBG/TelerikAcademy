function createImagesPreviewer(selector, items) {
    ...
    filterBox.oninput = function () {
        ...
        var elements = rightPanel.getElementsByTagName("li");
        for (var i = 0; i < elements.length; i++) {
            var currElement = elements[i];
            var currElementInnerText = currElement.getElementsByTagName("span")[0].innerText;
            if (currElementInnerText.indexOf(filterText) == -1) {
    ...
    imgsList.onclick = function (element) {
        var newSrc = element.target.getAttribute("src");
        if (newSrc) {
