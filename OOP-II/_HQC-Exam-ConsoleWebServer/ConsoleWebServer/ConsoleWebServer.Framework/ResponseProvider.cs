namespace ConsoleWebServer.Framework
{
    ...
    public class ResponseProvider
    {
        private HttpResponse Process(HttpRequest request)
        {
            ...
            else if (request.Method.ToLower() == "head")
            {
                return new HttpResponse(request.ProtocolVersion, HttpStatusCode.OK, "");
            }
        ...
        private Controller CreateController(HttpRequest request)
        {
            ...
            var type = Assembly.GetEntryAssembly().GetTypes().FirstOrDefault(
            ...
            if (type == null)
            {
                throw new HttpNotFoundException(string.Format(
                    "Controller with name {0} not found!", controllerClassName));
            }
